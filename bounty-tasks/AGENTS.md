...
Admin is determined by the `ADMIN_EMAILS` runtime var — a comma-separated list of admin emails (case-insensitive). Users with a verified email matching this list get the Admin tab and admin-only API routes.

Ask the owner which email(s) should be admin, then:

```bash
edgespark var set ADMIN_EMAILS "alice@acme.com,bob@acme.com"
```

If `ADMIN_EMAILS` is unset, no user is admin.
